...
VALUE hunspell_suggest_auto(VALUE self, VALUE word) {
  ::Hunspell* ptr = ruby2Hunspell_HunspellPtr(self);
  if ( ! ptr ) return Qnil; // The exception is thrown by ruby2* 

  char **output = NULL;
  int ns = ptr->suggest_auto(&output, STR2CSTR(word));
...
VALUE hunspell_suggest_pos_stems(VALUE self, VALUE word) {
  ::Hunspell* ptr = ruby2Hunspell_HunspellPtr(self);
  if ( ! ptr ) return Qnil; // The exception is thrown by ruby2* 

  char **output = NULL;
  int ns = ptr->suggest_pos_stems(&output, STR2CSTR(word));
